...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    if (action === "session") {
      ...
      return res.json({ authenticated: await isAuthenticated(req) });
    }
    if (action === "logout") {
    ...
    if (action === "login") {
      ...
      if (!hash) {
        ...
        if (!initial)
          throw new Error(
            "Defina ADMIN_INITIAL_PASSWORD para realizar o primeiro acesso.",
          );
        if (password !== initial)
    ...
    if (action === "password") {
      if (!method(req, res, ["PUT"]) || !(await requireAuth(req, res))) return;
      ...
      await setConfig("admin_password_hash", await bcrypt.hash(next, 12));
      return res.json({ ok: true });
    }
