  ...
  return {
    ...
    items: table.rows.map(({ data, rowNumber }, order) => {
      ...
      return {
        ...
        topNotes: String(pick(data, ["Notas Topo", "Top Notes"])),
        heartNotes: String(pick(data, ["Notas Coração", "Notas Coracao", "Heart Notes"])),
        baseNotes: String(pick(data, ["Notas Base", "Base Notes"])),
        longevity: String(pick(data, ["Longevidade", "Longevity"])),
        sillage: String(pick(data, ["Rastro", "Sillage"])),
        rowNumber,
        raw: data,
        order,
      };
