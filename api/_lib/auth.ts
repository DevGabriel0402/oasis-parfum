...
function secret() {
  ...
  if (!value || value.length < 24)
    throw new Error("SESSION_SECRET não configurado com segurança.");
...
export async function createSession(res: VercelResponse) {
  ...
  res.setHeader(
    "Set-Cookie",
    `${COOKIE}=${token}; HttpOnly; SameSite=Lax; Path=/; Max-Age=2592000${secure}`,
  );
}
...
export async function isAuthenticated(req: VercelRequest) {
  ...
  try {
    ...
    return result.payload.sub === "oasis-admin";
  } catch {
    return false;
  }
}
